...
| `--io-buffers-size` | int      | `4096`   | `64`    | Размер буфера для bufio.Writer и bufio.Reader              |
| `--copy-buffers-size` | int    | `65536`  | `256`   | Размер буфера PipeCopy для копирования тела запроса        |

### Подробнее

- **`--io-buffers-size`** (по умолч. 4096, мин. 64):
  Задаёт размер буфера для bufio.Writer (writeRequestHeaders) и bufio.Reader (readResponseHeaders + streamResponseBody).
  При создании нового объекта из пула используется `bufio.NewWriterSize` / `bufio.NewReaderSize` с этим значением.
  Уже существующие объекты в пуле сохраняют старый размер. Изменение применяется только к вновь создаваемым.

- **`--copy-buffers-size`** (по умолч. 65536, мин. 256):
  Задаёт размер буфера для PipeCopy (копирование тела запроса в upstream).
  При смене значения пул `pipeCopyBufPool` пересоздаётся — старые буферы отбрасываются GC.
  Активные (взятые) буферы остаются валидными до возврата в пул.
